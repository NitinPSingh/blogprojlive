...
(function(a) {
    ...
    c.prototype.bind = function() {
        this.element.on("tokenize:load", {}, e.proxy(function() {
            this.init()
        }, this)).on("tokenize:clear", {}, e.proxy(function() {
            this.clear()
        }, this)).on("tokenize:remap", {}, e.proxy(function() {
            this.remap()
        }, this)).on("tokenize:select", {}, e.proxy(function(f, g) {
            this.focus(g)
        }, this)).on("tokenize:deselect", {}, e.proxy(function() {
            this.blur()
        }, this)).on("tokenize:search", {}, e.proxy(function(g, f) {
            this.find(f)
        }, this)).on("tokenize:paste", {}, e.proxy(function() {
            this.paste()
        }, this)).on("tokenize:dropdown:up", {}, e.proxy(function() {
            this.dropdownSelectionMove(-1)
        }, this)).on("tokenize:dropdown:down", {}, e.proxy(function() {
            this.dropdownSelectionMove(1)
        }, this)).on("tokenize:dropdown:clear", {}, e.proxy(function() {
            this.dropdownClear()
        }, this)).on("tokenize:dropdown:show", {}, e.proxy(function() {
            this.dropdownShow()
        }, this)).on("tokenize:dropdown:hide", {}, e.proxy(function() {
            this.dropdownHide()
        }, this)).on("tokenize:dropdown:fill", {}, e.proxy(function(g, f) {
            this.dropdownFill(f)
        }, this)).on("tokenize:dropdown:itemAdd", {}, e.proxy(function(g, f) {
            this.dropdownAddItem(f)
        }, this)).on("tokenize:keypress", {}, e.proxy(function(g, f) {
            this.keypress(f)
        }, this)).on("tokenize:keydown", {}, e.proxy(function(g, f) {
            this.keydown(f)
        }, this)).on("tokenize:keyup", {}, e.proxy(function(g, f) {
            this.keyup(f)
        }, this)).on("tokenize:tokens:reorder", {}, e.proxy(function() {
            this.reorder()
        }, this)).on("tokenize:tokens:add", {}, e.proxy(function(h, f, g, i) {
            this.tokenAdd(f, g, i)
        }, this)).on("tokenize:tokens:remove", {}, e.proxy(function(g, f) {
            this.tokenRemove(f)
        }, this))
    };
    c.prototype.init = function() {
        ...
        this.input = e('<input autocomplete="off" />').on("keydown", {}, e.proxy(function(f) {
            this.trigger("tokenize:keydown", [f])
        }, this)).on("keypress", {}, e.proxy(function(f) {
            this.trigger("tokenize:keypress", [f])
        }, this)).on("keyup", {}, e.proxy(function(f) {
            this.trigger("tokenize:keyup", [f])
        }, this)).on("focus", {}, e.proxy(function() {
            if (this.input.val().length >= this.options.searchMinLength && this.input.val().length > 0) {
                this.trigger("tokenize:search", [this.input.val()])
            }
        }, this)).on("paste", {}, e.proxy(function() {
        ...
        this.tokensContainer = e('<ul class="tokens-container form-control" />').addClass(this.element.attr("data-class")).attr("tabindex", this.options.tabIndex).append(this.searchContainer.append(this.input));
        if (this.options.placeholder !== false) {
            this.placeholder = e('<li class="placeholder" />').html(this.options.placeholder);
            this.tokensContainer.prepend(this.placeholder);
            this.element.on("tokenize:tokens:add tokenize:remap tokenize:select tokenize:deselect tokenize:tokens:remove", e.proxy(function() {
                if (this.container.hasClass("focus") || e("li.token", this.tokensContainer).length > 0 || this.input.val().length > 0) {
        ...
        this.container.focusin(e.proxy(function(f) {
            this.trigger("tokenize:select", [(e(f.target)[0] === this.tokensContainer[0])])
        }, this)).focusout(e.proxy(function() {
        ...
        this.element.on("tokenize:tokens:add tokenize:tokens:remove", e.proxy(function() {
            if (this.options.tokensMaxItems > 0 && e("li.token", this.tokensContainer).length >= this.options.tokensMaxItems) {
                this.searchContainer.hide()
            } else {
                this.searchContainer.show()
            }
        }, this)).on("tokenize:keydown tokenize:keyup tokenize:loaded", e.proxy(function() {
            this.scaleInput()
        }, this));
    ...
    c.prototype.reorder = function() {
        if (this.options.sortable) {
            var f, h, g = this.tokensContainer.tokenize2sortable("serialize").get(0);
            e.each(g, e.proxy(function(j, i) {
    ...
    c.prototype.paste = function() {
        var f = new RegExp(this.escapeRegex(Array.isArray(this.options.delimiter) ? this.options.delimiter.join("|") : this.options.delimiter), "ig");
        if (f.test(this.input.val())) {
            e.each(this.input.val().split(f), e.proxy(function(g, h) {
    ...
    c.prototype.tokenAdd = function(g, h, f) {
        ...
        if (e('option[value="' + g + '"]', this.element).length) {
            e('option[value="' + g + '"]', this.element).attr("selected", "selected").prop("selected", true)
        } else {
            if (f) {
        ...
        e('<li class="token" />').attr("data-value", g).append("<span>" + h + "</span>").prepend(e('<a class="dismiss" />').on("mousedown touchstart", {}, e.proxy(function(i) {
            i.preventDefault();
            if (i.which == 1) {
                this.trigger("tokenize:tokens:remove", [g])
            }
        }, this))).insertBefore(this.searchContainer);
                ...
                case a.BACKSPACE:
                    if (this.input.val().length < 1) {
                        ...
                        if (e("li.token.pending-delete", this.tokensContainer).length > 0) {
                            this.trigger("tokenize:tokens:remove", [e("li.token.pending-delete", this.tokensContainer).first().attr("data-value")])
                        } else {
    ...
    c.prototype.pressedDelimiter = function(f) {
        this.resetPending();
        if (this.isDropdownOpen() && e("li.active", this.dropdown).length > 0 && this.control === false) {
            f.preventDefault();
    ...
    c.prototype.dataSourceRemote = function(f) {
        this.debounce(e.proxy(function() {
    ...
    c.prototype.dataSourceLocal = function(h) {
        ...
        var i = this;
        e("option", this.element).not(":selected, :disabled").each(function() {
            if (j.test(i.transliteration(e(this).html()))) {
                k.push({
    ...
    c.prototype.dropdownShow = function() {
        if (!this.isDropdownOpen()) {
            ...
            this.dropdown = e('<div class="tokenize-dropdown dropdown"><ul class="dropdown-menu" /></div>').attr("data-related", this.id);
            e("body").append(this.dropdown);
            this.dropdown.show();
            this.dropdown.css("z-index", this.calculatezindex() + this.options.zIndexMargin);
            e(window).on("resize scroll", {}, e.proxy(function() {
                this.dropdownMove()
            }, this)).trigger("resize");
    ...
    c.prototype.dropdownFill = function(f) {
        if (f && f.length > 0) {
            ...
            if (this.options.displayNoResultsMessage) {
                this.trigger("tokenize:dropdown:show");
                this.trigger("tokenize:dropdown:clear");
                this.dropdown.find(".dropdown-menu").append(e('<li class="dropdown-item locked" />').html(this.options.noResultsMessageText.replace("%s", this.input.val())))
            } else {
                this.trigger("tokenize:dropdown:hide")
            }
        }
    ...
    c.prototype.dropdownAddItem = function(f) {
        if (this.isDropdownOpen()) {
            var g = e('<li class="dropdown-item" />').html(this.dropdownItemFormat(f)).on("mouseover", e.proxy(function(h) {
                h.preventDefault();
                h.target = this.fixTarget(h.target);
                e("li", this.dropdown).removeClass("active");
                e(h.target).parent().addClass("active")
            }, this)).on("mouseout", e.proxy(function() {
                e("li", this.dropdown).removeClass("active")
            }, this)).on("mousedown touchstart", e.proxy(function(h) {
                h.preventDefault();
                h.target = this.fixTarget(h.target);
                this.trigger("tokenize:tokens:add", [e(h.target).attr("data-value"), e(h.target).attr("data-text"), true])
            }, this));
            if (e('li.token[data-value="' + g.find("a").attr("data-value") + '"]', this.tokensContainer).length < 1) {
    ...
    c.prototype.dropdownItemFormat = function(h) {
        if (h.hasOwnProperty("text")) {
            ...
            if (this.options.searchHighlight) {
                var f = new RegExp((this.options.searchFromStart ? "^" : "") + "(" + this.escapeRegex(this.transliteration(this.lastSearchTerms)) + ")", "gi");
                g = h.text.replace(f, '<span class="tokenize-highlight">$1</span>')
            } else {
                g = h.text
            }
            return e("<a />").html(g).attr({
                "data-value": h.value,
                "data-text": h.text
            })
        }
    ...
    c.prototype.clear = function() {
        e.each(e("li.token", this.tokensContainer), e.proxy(function(g, f) {
    ...
    c.prototype.scaleInput = function() {
        ...
        var f, g;
        this.ctx.font = this.input.css("font-style") + " " + this.input.css("font-variant") + " " + this.input.css("font-weight") + " " + Math.ceil(parseFloat(this.input.css("font-size"))) + "px " + this.input.css("font-family");
        f = Math.round(this.ctx.measureText(this.input.val() + "M").width) + Math.ceil(parseFloat(this.searchContainer.css("margin-left"))) + Math.ceil(parseFloat(this.searchContainer.css("margin-right")));
        g = this.tokensContainer.width() - (Math.ceil(parseFloat(this.tokensContainer.css("border-left-width"))) + Math.ceil(parseFloat(this.tokensContainer.css("border-right-width")) + Math.ceil(parseFloat(this.tokensContainer.css("padding-left"))) + Math.ceil(parseFloat(this.tokensContainer.css("padding-right")))));
        if (f >= g) {
            f = g
        }
    ...
    c.prototype.escape = function(f) {
        ...
        g.innerHTML = f;
        f = (g.textContent || g.innerText || "");
        return String(f).replace(/["]/g, function() {
            return '"'
        })
    };
...
! function(c, f, a, e) {
    ...
    m.prototype = {
        dragInit: function(u, t) {
            ...
            if (d.length) {
                ...
                if (this.item.is(this.options.exclude) || !this.options.onMousedown(this.item, o.onMousedown, u)) {
                    return
                }
                this.setPointer(u);
        ...
        drag: function(w) {
            ...
            if (!u || u.top - d > v.top || u.bottom + d < v.top || u.left - d > v.left || u.right + d < v.left) {
        ...
        getContainerDimensions: function() {
            if (!this.containerDimensions) {
                n(this.containers, this.containerDimensions = [], this.options.tolerance, !this.$getOffsetParent())
            }
    ...
    j.prototype = {
        ...
        getItemOffsetParent: function() {
            var t, d = this.el;
            if (d.css("position") === "relative" || d.css("position") === "absolute" || d.css("position") === "fixed") {
                t = d
            } else {
                t = d.offsetParent()
            }
        ...
        _serialize: function(v, u) {
            var w = this,
                d = u ? "item" : "container",
                t = this.$getChildren(v, d).not(this.options.exclude).map(function() {
